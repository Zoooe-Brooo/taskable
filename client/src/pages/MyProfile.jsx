...
const MyProfile = () => {
  ...
  const { profile, status, error, favoriteServices } = useSelector(selectUserData);
    ...
    if (!Auth.loggedIn()) {
      ...
      if (savedFavorites) {
        try {
          ...
          dispatch({ type: 'user/setFavoriteServices', payload: parsedFavorites });
        } catch (error) {
  ...
  if (status === 'loading') {
    return (
      <Box p={10} bgGradient="linear(to-tl, #3AAFA9, #2B7A78)" minH="100vh">
        <Text color="white" fontSize="xl" textAlign="center">Loading profile...</Text>
      </Box>
    );
  }

  if (error && error !== "Cannot read properties of null (reading 'orders')") {
    return (
      <Box p={10} bgGradient="linear(to-tl, #3AAFA9, #2B7A78)" minH="100vh">
        <Text color="white" fontSize="xl" textAlign="center">
          Error loading profile: {error}
        </Text>
        <Button onClick={() => dispatch(fetchUserProfile())} mt={4} colorScheme="teal">
          Retry
        </Button>
      </Box>
    );
  ...
  if (!profile) {
    return (
      <Box p={10} bgGradient="linear(to-tl, #3AAFA9, #2B7A78)" minH="100vh">
        <Text color="white" fontSize="xl" textAlign="center">No profile data available</Text>
      </Box>
    );
  }
        ...
        <Box>
          ...
          >
            ...
            <Box position="relative" width="100%">
              ...
              />
              <Text color="white" position="absolute" right="0" top="-25px" fontSize="sm">
                75% Complete
              </Text>
            </Box>
          </Box>
